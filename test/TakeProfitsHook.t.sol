...
// We have our main contract -> TakeProfitsHook.sol
// We are going to create a stub contract -> TakeProfitsHookStub.sol
// This will inherit from our main contract, but override the validate hook address function
// In our testing environment, we'll use cheat codes to overwrite the storage (i.e. the contract code)
// in the main contract with our stub implementation

// Foundry libraries
import "forge-std/Test.sol";
import {GasSnapshot} from "forge-gas-snapshot/GasSnapshot.sol";

// Test ERC-20 token implmementation
...
import {TakeProfitsStub} from "../src/TakeProfitsHookStub.sol";
...
contract TakeProfitsHookTest is Test, GasSnapshot {
    ...
    // poolKey and poolId are the pool key and pool id for the poolManager
    PoolKey poolKey;
    PoolId poolId;
    ...
    // as we add amounts of token0 and token1 to the pool during setUp
    ...
    function test_orderExecute_zeroForOne() public {
        ...
        int256 tokensLeftToSell = hook.takeProfitPositions(poolId, tick, zeroForOne);
        assertEq(tokensLeftToSell, 0);

        // Check that the hook contract has the expected numer of token1 tokens ready to redeem
        uint256 tokenId = hook.getTokenId(poolKey, tickLower, zeroForOne);
